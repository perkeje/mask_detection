...
def main(image_folder, method='hog_svm', output_json='detections.json'):
    num = 0
    json_output = []
    ...
    for filename in filenames:
        if filename.endswith(".jpg"):
            ...
            for detection in detections:
                coords = np.round(detection[:4]).astype(int)
                json_output.append({
                    "id": detection_idx + 1,
                    "image_id": num +1,
                    "area": float(calculate_area(coords)),
                    "category_id": 1,
                    "bbox": [float(i) for i in coords],
                    "segmentation": [[float(i) for i in seg] for seg in bbox_to_segmentation(coords)],
                    "iscrowd": 0,
...
main('/Users/jperic/Downloads/lica iz maski.v1i.voc/test', 'hog_svm','eval/results/hog_svm.json')
